...
class ModelBasedSimulator(UserSimulator):
    ...
    def _sample_action(self):
        ...
        self.state['diaact'] = random.choice(tuple(dialog_config.start_dia_acts.keys()))

        # "sample" informed slots
        if len(self.goal['inform_slots']) > 0:
            known_slot = random.choice(tuple(self.goal['inform_slots'].keys()))
    ...
    def next(self, s, a):
        ...
        if action['diaact'] == 'inform':
            if len(action['inform_slots'].keys()) > 0:
                slots = tuple(action['inform_slots'].keys())[0]
